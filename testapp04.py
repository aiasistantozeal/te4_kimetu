...
def main():
    ...
    # タイトルの表示
    st.title("画像が「鬼滅の刃」の「柱」か「鬼殺隊員」か「上弦の鬼」か「下弦の鬼」かを判別します。")
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  # 柱の確率(%)
        pred1 = round(prediction[1],3) * 100  # 鬼殺隊員の確率(%)
        pred2 = round(prediction[2],3) * 100  # 上弦の鬼の確率(%)
        pred3 = round(prediction[3],3) * 100  # 下弦の鬼の確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「柱」かも！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「鬼殺隊員」かも！")
        elif classNo == 2:
            st.subheader(f"これは{pred2}％の確率で「上弦の鬼」かも！")
        else:
            st.subheader(f"これは{pred3}％の確率で「下弦の鬼」かも！")
...
#アプリの概要：「鬼滅の刃」の「柱」「鬼殺隊員」「上弦の鬼」「下弦の鬼」を認識します。
#使い方:画像をアップロードすると結果が表示されます。
#感想：ノーコードでAIのモデルがつくれるTeachable Machine、簡単に動的サイトをつくれるStreamlitはとても便利だと思いました。画像を学習させるときは偏りがないように枚数をなるべく統一しました。
#
#=============================================================================#
